...
<nav class="navbar navbar-expand-sm bg-white navbar-white fixed-top" style="border-bottom: solid 1px gray;">
	...
	<div class="container mt-3">
			<h1 class="h3 fw-normal mb-5 text-center" style="margin-top: 100px;">D
			E T A I L - ${dto.qna_subject}</h1>
		...
		<section>
			...
			<div class="mb-3">
				<label for="name">작성자:</label> <span class="form-control">${dto.user_num.user_id}
					(${dto.user_num.user_id})</span>
			</div>
			<div class="mb-3 mt-3">
				<label for="subject">제목:</label> <span class="form-control">${dto.qna_subject}</span>
			</div>
			<div class="mb-3 mt-3">
				<label for="content">내용:</label> <span class="form-control">${dto.qna_content}</span>
			</div>
					...
					<ul class="list-group mb-1">
						<c:forEach var="fileDto" items="${fileList}">
							<li class="list-group-item"><a
								href="download?id=${fileDto.file_num}">${fileDto.file_source}</a></li>
						</c:forEach>
					</ul>
					<%-- 이미지인 경우 보여주기 --%>
					<c:forEach var="fileDto" items="${fileList}">
						<c:if test="${fileDto.image == true }">
							<div>
								<img
									src="${pageContext.request.contextPath}/upload/${fileDto.file_name}"
									class="img-thumbnail">
							</div>
						</c:if>
					</c:forEach>
			...
			<!-- 하단 링크 -->
			<c:if
				test="${fn:contains(PRINCIPAL.authorities, 'ROLE_MEMBER' ) && (PRINCIPAL.user_num == dto.user_num.user_num)}">
				<a class="btn btn-outline-dark" href="update?num=${dto.qna_num }">수정</a>
			</c:if>

			<a class="btn btn-outline-dark"
				href="list?page=${page != null ? page : '' }">목록</a>
